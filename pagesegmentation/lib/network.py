...
class Network:
    def __init__(self,
        ...
        if model and continue_training or model and self.type == 'Predict':
            ...
            optimizer = None
            if optimizer_norm_clipping and optimizer_clip_value:
                optimizer = tf.keras.optimizers.Adam(lr=l_rate, clipnorm=optimizer_norm_clip_value,
                                                     clipvalue=optimizer_clip_value)
            else:
                if optimizer_norm_clipping:
                    optimizer = tf.keras.optimizers.Adam(lr=l_rate, clipnorm=optimizer_norm_clip_value)
                elif optimizer_clipping:
                    optimizer = tf.keras.optimizers.Adam(lr=l_rate, clipvalue=optimizer_clip_value)
                else:
    ...
    def train_dataset(self, setting: TrainSettings = None,
        ...
        if setting.early_stopping_max_l_rate_drops != 0:
            early_stop_cb = tf.keras.callbacks.EarlyStopping(monitor=setting.monitor,
